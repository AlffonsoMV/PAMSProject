...
This chapter explores the simulation of random variables, a cornerstone in statistical and dynamical system studies. In Section~\ref{sec:inverse_cdf} we introduce the Inverse CDF method providing a foundational overview of its mechanics and application in simulating random variables.
...
\section{Empirical validation of important statistical laws}

In this section, we will empirically validate the Law of Large Numbers and the Central Limit Theorem. These laws are pivotal in probability theory and have profound implications in statistical analysis and modeling. The empirical validation of these laws will serve to corroborate their theoretical underpinnings and substantiate their practical relevance in simulation scenarios.
...
See Figure~\ref{fig:comparisonvariancereduction} for a visual representation of the error in relation to the theoretical value as we increase the number of trials for each method. Note that for this comparision we have used \(10\) stratas for the stratified sampling method.
...
\begin{center}
	\begin{tabular}{ |p{5cm}||p{3cm}|p{3cm}|p{3cm}|  }
		...
		\hline
		\hline
		Direct sampling   			& \(1.95839 \cdot 10^{-4}\) \\
		\hline
		Control variates sampling 	& \(1.12095 \cdot 10^{-5}\) \\
		\hline
		Importance sampling      	& \(2.17864 \cdot 10^{-7}\) \\
		\hline
		Stratified sampling       	& \(2.76024 \cdot 10^{-6}\) \\
		\hline
		Antithetic sampling      	& \(2.14899 \cdot 10^{-5}\) \\
		\hline
	\end{tabular}
\end{center}
...
Upon examining the table, notable findings emerge. Initially, it's evident that direct sampling ranks as the least effective method, aligning with our theoretical insights. This approach primarily aims to reduce the variance in our estimates. In contrast, importance sampling emerges as the superior technique. However, it demands greater effort, as it involves identifying a more compatible distribution for the function we aim to integrate. Stratified sampling comes in as the next best option, showing potential for improved outcomes with an increased number of strata. Nevertheless, drawing definitive conclusions from a single case study is premature; further experimentation across various problems is necessary to determine the optimal method for each situation.
...
The final objective of the Chapter~\ref{sec:markov_chains} is to introduce the Markov Chain Monte Carlo Methods (MCMC). As first step to introduce MCMC, we need to introduce the concept of stationary distribution. 
...
\label{sec:ergodic_theorem}

After discussing the fundamental theorem and stationary distributions, it is crucial to introduce the Ergodic Theorem for Markov Chains. This theorem is a cornerstone in understanding the long-term behavior of Markov Chains.

\begin{theorem}[Ergodic Theorem for Markov Chains]
	Let $(X_n)_{n \geq 0}$ be an irreducible and aperiodic Markov Chain on a finite state space $E$ with a stationary distribution $\pi$. Then, for any initial distribution $\mu$ and for any function $f : E \to \mathbb{R}$, we have

	\[
		\lim_{n \to \infty} \frac{1}{n} \sum_{i=1}^n f(X_i) = \sum_{x \in E} f(x) \pi(x) \quad \text{with probability 1}.
	\]
\end{theorem}

This theorem asserts that the time average of a function along the paths of the chain converges to a space average, weighted by the stationary distribution. This result is pivotal for the convergence analysis of Markov Chain Monte Carlo methods.

A detailed proof and further discussion can be found in \cite{cambridge_markov}.
...
\label{sec:boltzmann_distribution}

Prior to delving into the application of the Metropolis-Hastings and Gibbs Sampling algorithms to the Müller-Brown potential, a natural query arises: given that the Müller-Brown potential is essentially a function describing energy, how does it relate to the concept of sampling a probability distribution? The answer lies in the realm of the Boltzmann distribution.

The Boltzmann Distribution plays a crucial role in connecting the concept of a potential, such as the Müller-Brown potential, to a probability distribution suitable for sampling methods. This distribution is pivotal in statistical mechanics and provides a fundamental bridge between the microstate of a system (described by the potential energy) and its macrostate (described by temperature and entropy).

The Boltzmann distribution is expressed as follows:

\[
    \pi(x, y) = \frac{e^{-\frac{U(x, y)}{kT}}}{Z}
\]

In this equation, \( \pi(x, y) \) denotes the probability of the system being in a state characterized by coordinates \( (x, y) \). The function \( U(x, y) \) represents the potential energy of that state, which in this context is given by the Müller-Brown potential. The temperature of the system is denoted by \( T \), and \( k \) is the Boltzmann constant, which acts as a bridge between the macroscopic and microscopic worlds.

The denominator, \( Z \), is the partition function, a crucial component in statistical mechanics. It serves as a normalizing factor, ensuring that the probabilities across all possible states sum up to 1. Mathematically, it's defined as an integral over all states:

\[
    Z = \int \int e^{-\frac{U(x, y)}{kT}} \, dx \, dy
\]

The integration is performed over the entire configuration space, accounting for all possible states \( (x, y) \).

The Boltzmann distribution's importance cannot be overstated in the context of sampling algorithms like Metropolis-Hastings and Gibbs Sampling. By converting the potential energy landscape into a probability distribution, these algorithms can effectively explore the state space, focusing on regions of high probability (low energy) while still allowing for exploration of less probable (higher energy) states. This approach is not only pivotal for the Müller-Brown potential but forms the foundation for the discussion of physical potentials in statistical mechanics and is extensively utilized in Chapter~\ref{sec:markov_chains}.
